...
def batch_upsert(microBatchDF, batchId):
    window = Window.partitionBy("order_id", "customer_id").orderBy(F.col("_commit_timestamp").desc()) # using a composite key of ("order_id", "customer_id") for partition and sort the records in the window by timestamp
    ...
    query = """
        ...
        ON c.order_id=r.order_id AND c.customer_id=r.customer_id -- using a composite key of ("order_id", "customer_id") for join
...
def process_customers_orders():
    ...
    query = (orders_df # perform a join between 2 streaming tables based on the customer_id
